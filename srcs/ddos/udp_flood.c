...
void *GenerateUdpFlood(void *data)
{
	...
	while (1) {
		...
		ipv4_h.check = IphdrGetChecksum((__u16 *) &ipv4_h,sizeof(struct udphdr) + sizeof(struct icmp));
		// make udp header
		...
		char *packet = AssembleIphdrWithData(ipv4_h, udp_h_ptr, sizeof(struct udphdr));
