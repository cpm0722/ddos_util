...
char g_resbuf_now_src_ip[16];
char g_resbuf_now_dest_ip[16];
unsigned int g_resbuf_now_dest_port;
// thread
...
void response_buffering_print_usage(void) {
	printf(
			"response buffering Usage : [Src-IP/mask] [Dest-IP/mask] [Dest-Port] [#Requests-Per-Sec]\n");
	return;
}
...
void* generate_response_buffering(void *data) {
	...
	while (1) {
		...
		// get now resource
		generator(g_resbuf_src_ip, g_resbuf_dest_ip, g_resbuf_src_mask,
				g_resbuf_dest_mask, g_resbuf_dest_port_start,
				g_resbuf_dest_port_end, g_resbuf_now_src_ip,
				g_resbuf_now_dest_ip, &g_resbuf_now_dest_port);
...
void response_buffering_main(char *argv[]) {
	...
	split_ip_mask_port(argv, g_resbuf_src_ip, g_resbuf_dest_ip,
			&g_resbuf_src_mask, &g_resbuf_dest_mask, &g_resbuf_dest_port_start,
