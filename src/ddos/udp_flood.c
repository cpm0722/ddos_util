...
// session counting
unsigned long g_udp_num_total;
unsigned long g_udp_num_generated_in_sec;
// from main()
char g_udp_src_ip[16] = { 0, };
char g_udp_dest_ip[16] = { 0, };
unsigned int g_udp_src_mask;
unsigned int g_udp_dest_mask;
unsigned int g_udp_dest_port_start;
unsigned int g_udp_dest_port_end;
unsigned int g_udp_request_per_sec;
...
void *generate_udp_flood(void *data)
{
	...
	while (1) {
		...
		// time checking
		time_check(&g_udp_mutex, &g_udp_cond, &g_udp_before_time, &g_udp_now_time, &g_udp_num_generated_in_sec);
